...
class Mazmorra:
    """
    Clase que representa una mazmorra en el juego donde un héroe enfrenta monstruos y encuentra tesoros.
    ...
    def __init__(self, heroe):
        ...
        self.monstruos = [
            Monstruo(),
            Monstruo(),
            Monstruo("Orco", 20, 10, 60)  # Lista de enemigos inicial
        ]

        # Lista de dificultad media de enemigos:
        # [Monstruo(), Monstruo(), Monstruo("Orco", 20, 10, 60), Monstruo("Orco", 20, 10, 60),
        # Monstruo("Terrible Murciélago", 25, 20, 70)]

        # Lista mortal de enemigos:
        # [Monstruo("Terrible Murciélago", 30, 20, 70), Monstruo("Terrible Murciélago", 30, 20, 70), 
        # Monstruo("Terrible Murciélago", 30, 20, 70), Monstruo("Terrible Murciélago", 30, 20, 70)]
    ...
    def jugar(self):
        ...
        while self.heroe.esta_vivo() and self.monstruos:  # Mientras el héroe esté vivo y haya monstruos
            ...
            monstruo_aux = random.choice(self.monstruos)  # Selección aleatoria de un monstruo
    ...
    def enfrentar_enemigo(self, enemigo):
        ...
        """
        while enemigo.esta_vivo() and self.heroe.esta_vivo():  # Mientras el enemigo y el héroe estén vivos
            eleccion = 0
            while eleccion not in (1, 2, 3):  # Validar la elección del jugador
                try:
            ...
            if enemigo.esta_vivo():  # Si el enemigo sigue vivo, ataca al héroe
                enemigo.atacar(self.heroe)  # Monstruo ataca al héroe
                if self.heroe.defensaAumentada:
                    self.heroe.reset_defensa()  # Restablece la defensa si estaba aumentada

            if not enemigo.esta_vivo():  # Si el enemigo ha sido derrotado
    ...
    def buscar_tesoro(self):
        ...
        print("Buscando tesoro...")  # Mensaje informando que se busca un tesoro
